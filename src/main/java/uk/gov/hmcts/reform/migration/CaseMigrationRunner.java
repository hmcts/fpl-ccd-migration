...
public class CaseMigrationRunner implements CommandLineRunner {
    ...
    @Value("${migration.caseIdFile}")
    private String caseIdFile;
    ...
    public void run(String... args) {
        log.info("Customised CaseMigrationRunner started (reading caseIdFile) ...");
        try {
            ...
            } else if (caseIdFile != null) {
                log.info("Data migration of multiple cases started");
                // read file line by line
                List<String> caseIds = Files.readAllLines(new File(caseIdFile).toPath(), Charset.defaultCharset());

                caseIds.parallelStream().forEach(id -> {
                    if (!id.equals("")) {
                        caseMigrationProcessor.processSingleCase(userToken, id);
                    }
                });
            }  else {
